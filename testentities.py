...
class knights(entities.entities):
    def __init__(self, initial=None):
        self.index = True
        self.indexes += entities.index(name='name', 
                        keyfn=lambda k: k.name, 
                        prop='name')

        self.indexes += entities.index(name='traittype', 
...
class knight(entities.entity):
    ...
    def brokenrules(self):
        ...
        if type(self.name) != str:
            brs += entities.brokenrule("Names must be strings")
        return brs
...
class philosopher(entities.entity):
    def __init__(self, name):
        self.name = name

class performer(entities.entity):
    def __init__(self, name):
        self.name = name
...
class constant(entities.entity):
    def __init__(self, v):
        self.value = v
...
class entities_(tester.tester):
    ...
    def it_raises_exception_when_accessing_indexes(self):
        es = entities.entities()
        self.expect(ValueError, lambda: es.indexes)

        es.index = True
        self.expect(None, lambda: es.indexes)
    ...
    def it_instantiates(self):
        ...
        e1, e2 = entities.entity(), entities.entity()
    ...
    def it_clears(self):
        ...
        es = entities.entities([entities.entity(), entities.entity()])
        es.clear()
...
class table_(tester.tester):
    def it_calls__init__(self):
        ...
        tbl = table.table(x=10, y=20, initval=k)
    ...
    def it_calls__iter__(self):
        tbl = createtable(5, 5)
        for r in tbl:
            self.assertEq(table.row, type(r))
            self.assertEq(5, r.fields.count)

    def it_calls__getitem__(self):
        tbl = createtable(5, 5)
        for i in range(5):
            self.assertEq(table.row, type(tbl[i]))
    ...
    def it_gets_columns(self):
        tbl = createtable(5, 5)
        cs = tbl.columns
        self.assertEq(table.columns, type(cs))
    ...
    def it_gets_fields(self):
        ...
        self.assertEq(table.fields, type(fs))
        ...
        tbl[0][0] = rst = table.field(the4.first)
        tbl[0].fields.second = nd = table.field(the4.second)
        tbl[0].fields[2:4] = rd, rth = table.fields(initial=the4[2:4])
    ...
    def it_calls_slice(self):
        ...
        try:
            tbl.slice(center=entities.entity(), radius=123)
        ...
        f = tbl[0][0]
        s = tbl.slice(f, 1)
        self.assertIs(table.table, type(s))
...
class table_columns(tester.tester):
...
class table_column(tester.tester):
    def it_calls__init(self):
        ...
        self.assertEq(table.columns, type(cs))
        for i, c in enumerate(cs):
            self.assertEq(table.column, type(c))
            fs = c.fields
            self.assertEq(5, fs.count)
            for j, f in enumerate(fs):
                self.assertEq(table.field, type(f))
                y, x = j, i
                self.assertEq([y, x], f.value)
...
class field(tester.tester):
    ...
    def it_gets__str__(self):
        tbl = table.table(5, 5, initval='123')
        for f in tbl.fields:
