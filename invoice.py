...
class item(orm.entity)
    ...
    # The quantity of the invoice item. The ``quantity`` is optional
    # because there may only be an ``amount`` and not a ``quantity`` for
    # the ``item`` such as ``features`` where the ``quantity`` really isn't
    # necessary and is not applicable.
    quantity = dec
    amount = dec
    description = text
...
class type(orm.entity):
    ...
    item", or "invoice time entry item".
    """
